...
import edu.byu.imaal.dnscapture.activities.ErrorDialogActivity;
import edu.byu.imaal.dnscapture.database.DatabaseHelper;
import edu.byu.imaal.dnscapture.util.DataSavingSentryEventProcessor;
 ...
 */
public class DNSCollector extends Application {
    ...
    private final Thread.UncaughtExceptionHandler customHandler = new Thread.UncaughtExceptionHandler() {
        ...
        public void uncaughtException(Thread t, Throwable e) {
            ...
            LogFactory.writeMessage(DNSCollector.this, new String[]{LOG_TAG, LogFactory.Tag.ERROR.toString()}, "Caught uncaught exception");
            LogFactory.writeStackTrace(DNSCollector.this, new String[]{LOG_TAG, LogFactory.Tag.ERROR.toString()}, e);
            if (showErrorDialog(e)) {
                ErrorDialogActivity.show(DNSCollector.this, e);
                System.exit(2);
            }
    ...
    private boolean showErrorDialog(Throwable exception) {
        ...
        return exception.getMessage() != null && (exception.getMessage().toLowerCase().contains("cannot create interface") || exception.getMessage().toLowerCase().contains("failed to detect default dns server"));
    }
                ...
                public void run() {
                    try {
                        ...
                        if(enabled) {
                            ...
                            SentryAndroid.init(DNSCollector.this, new Sentry.OptionsConfiguration<SentryAndroidOptions>() {
                                @Override
